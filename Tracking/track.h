...
public:
    ...
    void Update(const Point_t& pt, const CRegion& region, bool dataCorrect, size_t max_trace_length)
	{
        if (m_filterObjectSize)
        {
            ...
            m_predictionPoint.x = (m_predictionRect.tl().x + m_predictionRect.br().x) / 2;
            m_predictionPoint.y = (m_predictionRect.tl().y + m_predictionRect.br().y) / 2;
        }
        else // Kalman filter only for object center
        {
            ...
            if (pointsCount)
            {
                if (dataCorrect)
                {
                    m_predictionPoint.x = (pt.x + averagePoint.x) / 2;
                    m_predictionPoint.y = (pt.y + averagePoint.y) / 2;
                    m_predictionPoint = m_kalman->Update(m_predictionPoint, dataCorrect);
                }
                else
                {
